...
function contract(C, A, X)
    ...
    isempty(constrained) && return emptyinterval(X)

    return IntervalBox(C.backward(X, constrained, intermediate) )
end

(C::Contractor)(A::Interval, X) = C(IntervalBox(A), X)
(C::BasicContractor)(A::Interval, X) = C(IntervalBox(A), X)

(C::Contractor)(A::IntervalBox, X) = contract(C, A, X)
(C::BasicContractor)(A::IntervalBox, X) = contract(C, A, X)
