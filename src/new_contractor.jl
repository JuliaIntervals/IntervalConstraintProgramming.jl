...
Contractor(vars::V, ex::E, contractor::C, parameters::P) where {V, E, C, P} = Contractor{!isempty(parameters), V, E, C, P}(vars, ex, contractor, parameters)
...
function Contractor(ex, vars, params)
    if isempty(params)
        return Contractor(vars, ex, forward_backward_contractor(ex, vars), Sym[])
    else
        return Contractor(vars, ex, forward_backward_contractor(ex, vars, params), params)
    end
end
...
struct Separator{HasParameters, V, E, C, R, P} <: AbstractSeparator
    vars::V
    ex::E
    constraint_set::C
    contractor::R
