...
mutable struct TrackedReal{V<:Real,O} <: Real #Mutable type to track the input and other variables
    ...
    index::Int
    origin::O
    TrackedReal{V,O}(value, tape, index, origin) where {V,O} = new{V,O}(value, tape, index, origin)
    TrackedReal{V,O}(value, tape) where {V,O} = new{V,O}(value, tape, NULL_INDEX)
    TrackedReal{V,O}(value) where {V,O} = new{V,O}(value, NULL_TAPE, NULL_INDEX)
end
...
struct TrackedArray{V,N,VA} <: AbstractArray{TrackedReal{V,TrackedArray{V,N,VA}},N}
    value::VA
    tape::InstructionTape
    function TrackedArray{V,N,VA}(value::AbstractArray{V,N},
...
idstr(x) = string(base(62, objectid(x)))[1:3]
